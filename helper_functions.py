...
def retrieve_context_per_question(question, chunks_query_retriever):
    ...
    Args:
        ...
        chunks_query_retriever: The retriever object (callable)
    ...
    """
    try:
        # Get relevant documents using the retriever (call it directly)
        relevant_docs = chunks_query_retriever(question)
        
        # Extract page content and URLs
        context = "\n\n".join([doc.page_content for doc in relevant_docs])
        urls = list(set([doc.metadata.get('source', '') for doc in relevant_docs if doc.metadata.get('source')]))
        
        return context, urls
    except Exception as e:
        print(f"Error in retrieve_context_per_question: {str(e)}")
        return "", []
